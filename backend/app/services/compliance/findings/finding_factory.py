...
_DEFINITIONS: dict[str, tuple[str, str, str, str]] = {
    ...
    FindingCode.LOW_TRANSLATION_SIMILARITY: (
        FindingType.TRANSLATION_SIMILARITY,
        FindingSeverity.MAJOR,
        "Translation similarity is low",
        "Review the aligned translations; similarity is only a review signal.",
    ),
    FindingCode.TRANSLATION_SIMILARITY_NEEDS_REVIEW: (
        FindingType.TRANSLATION_SIMILARITY,
        FindingSeverity.MINOR,
        "Translation similarity needs review",
        "Review the aligned language pair and its provenance.",
    ),
    FindingCode.TRANSLATION_NOT_EVALUATED: (
        FindingType.TRANSLATION_SIMILARITY,
        FindingSeverity.INFORMATION,
        "Translation similarity was not evaluated",
        "Review prerequisites, language presence, and model readiness.",
    ),
    FindingCode.TRANSLATION_LENGTH_RATIO_ANOMALY: (
        FindingType.TRANSLATION_SIMILARITY,
        FindingSeverity.MINOR,
        "Translation length ratio is anomalous",
        "Review whether content is missing or grouped incorrectly.",
    ),
    FindingCode.TRANSLATION_NUMBER_MISMATCH: (
        FindingType.TRANSLATION_SIMILARITY,
        FindingSeverity.MAJOR,
        "Translation contains a number mismatch",
        "Verify numbers and formatting in each language.",
    ),
    FindingCode.TRANSLATION_DATE_MISMATCH: (
        FindingType.TRANSLATION_SIMILARITY,
        FindingSeverity.MAJOR,
        "Translation contains a date mismatch",
        "Verify unambiguous dates in each language.",
    ),
    FindingCode.TRANSLATION_MEASUREMENT_MISMATCH: (
        FindingType.TRANSLATION_SIMILARITY,
        FindingSeverity.MAJOR,
        "Translation contains a measurement mismatch",
        "Verify values and measurement units without unsafe conversion.",
    ),
    FindingCode.TRANSLATION_REFERENCE_MISMATCH: (
        FindingType.TRANSLATION_SIMILARITY,
        FindingSeverity.MAJOR,
        "Translation contains a reference mismatch",
        "Verify section, clause, and document references.",
    ),
    FindingCode.TRANSLATION_NEGATION_MISMATCH: (
        FindingType.TRANSLATION_SIMILARITY,
        FindingSeverity.MAJOR,
        "Translation may contain a negation mismatch",
        "Review this automated negation signal in full context.",
    ),
    FindingCode.TRANSLATION_CONTENT_TOO_SHORT: (
        FindingType.TRANSLATION_SIMILARITY,
        FindingSeverity.INFORMATION,
        "Translation content is too short for reliable comparison",
        "Review the text manually or provide sufficient content.",
    ),
    FindingCode.TRANSLATION_MODEL_UNAVAILABLE: (
        FindingType.TRANSLATION_SIMILARITY,
        FindingSeverity.INFORMATION,
        "Local similarity model is unavailable",
        "Install and verify the configured local model, then re-run.",
    ),
    FindingCode.NON_PREFERRED_GLOSSARY_TERM: (
        FindingType.GLOSSARY,
        FindingSeverity.MINOR,
        "A non-preferred glossary term is used",
        "Use an approved preferred term unless an exception applies.",
    ),
    FindingCode.FORBIDDEN_GLOSSARY_TERM: (
        FindingType.GLOSSARY,
        FindingSeverity.MAJOR,
        "A forbidden glossary term is used",
        "Replace the term or record an approved scoped exception.",
    ),
    FindingCode.MISSING_GLOSSARY_TRANSLATION: (
        FindingType.GLOSSARY,
        FindingSeverity.MAJOR,
        "A required glossary translation is missing",
        "Add the approved term in the missing language.",
    ),
    FindingCode.INCONSISTENT_GLOSSARY_TRANSLATION: (
        FindingType.GLOSSARY,
        FindingSeverity.MINOR,
        "Glossary translation usage is inconsistent",
        "Use the approved term or allowed variant consistently.",
    ),
    FindingCode.REQUIRED_GLOSSARY_TERM_MISSING: (
        FindingType.GLOSSARY,
        FindingSeverity.MAJOR,
        "A required glossary concept is missing",
        "Add the required approved term where the concept applies.",
    ),
    FindingCode.GLOSSARY_TERM_LANGUAGE_MISMATCH: (
        FindingType.GLOSSARY,
        FindingSeverity.MINOR,
        "Glossary term appears in an unexpected language",
        "Verify language grouping and approved term usage.",
    ),
    FindingCode.GLOSSARY_MATCH_LOW_CONFIDENCE: (
        FindingType.GLOSSARY,
        FindingSeverity.INFORMATION,
        "Glossary match confidence is low",
        "Review the term match before taking corrective action.",
    ),
    FindingCode.GLOSSARY_EXCEPTION_EXPIRED: (
        FindingType.GLOSSARY,
        FindingSeverity.MINOR,
        "A glossary exception has expired",
        "Renew the exception with a reason or use the approved term.",
    ),
